...
import com.insaneio.insane.EQUAL_SIGN_STRING
...
fun String.decodeFromBase32(): ByteArray {
    ...
    require(trimmed.all { it in 'A'..'Z' || it in 'a'..'z' || it in '2'..'7' || it == '=' }) {
        "Data contains non-Base32 characters."
    }

    val paddingStartIndex = trimmed.indexOf('=')
    if (paddingStartIndex >= 0) {
        require(trimmed.substring(paddingStartIndex).all { it == '=' }) {
            "Padding characters are only allowed at the end."
        }
        require(trimmed.length % 8 == 0) {
            "Padded Base32 strings must have a length multiple of 8."
        }
        val paddingCount = trimmed.length - paddingStartIndex
        require(paddingCount in setOf(1, 3, 4, 6)) {
            "Invalid Base32 padding length."
        }
    }

    val data = trimmed.replace(EQUAL_SIGN_STRING, EMPTY_STRING)
    require(data.length % 8 in setOf(0, 2, 4, 5, 7)) {
